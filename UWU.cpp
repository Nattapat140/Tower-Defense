...
PlayScene* UWU::getPlayScene() {
	return dynamic_cast<PlayScene*>(Engine::GameEngine::GetInstance().GetActiveScene());
}
UWU::UWU() : Sprite("play/freeze.png", -100, Engine::GameEngine::GetInstance().GetScreenHeight() / 2), stage(0), timeTicks(0) {
	for (int i = 1; i <= 6; i++) {
		bmps.push_back(Engine::Resources::GetInstance().GetBitmap("play/light-" + std::to_string(i) + ".png"));
	}
	shockwave = Engine::Resources::GetInstance().GetBitmap("play/freezing.png");
	Velocity = Engine::Point(800, 0);
}
void UWU::Update(float deltaTime) {
	int phase;
	float scaleExp;
	switch (stage) {
	case 0:
	...
	case 1:
		...
		if (timeTicks >= timeSpanLight) {
			bmp = shockwave;
			stage++;
			AudioHelper::PlayAudio("freezing.ogg");
			break;
		}
	...
	case 2:
		...
		for (auto& it : getPlayScene()->EnemyGroup->GetObjects()) {
			...
			if (Engine::Collider::IsCircleOverlap(Position, CollisionRadius, enemy->Position, enemy->CollisionRadius))
				enemy->Slow(0.0000000000000000000000000000000000000001,3.5);
		}
		break;
	case 3:
